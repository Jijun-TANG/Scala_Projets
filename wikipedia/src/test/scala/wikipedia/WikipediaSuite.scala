...
class WikipediaSuite extends munit.FunSuite:
  ...
  test("'occurrencesOfLang' should work for (specific) RDD with several elements") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    val rdd = sc.parallelize(Seq(WikipediaArticle("title", "Java Jakarta"), WikipediaArticle("title", "CPP Bjourne"), WikipediaArticle("title", "Java virtual machines")))
    val res = (occurrencesOfLang("Java", rdd) == 2)
    assert(res, "occurrencesOfLang given (specific) RDD with 2 elements should equal to 2")
  }
