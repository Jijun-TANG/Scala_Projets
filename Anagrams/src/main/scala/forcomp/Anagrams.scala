...
object Anagrams extends AnagramsInterface:
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().groupBy((c:Char) => c).map((key,value)=>(key,value.length)).toList.sortWith(_._1<_._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x.map(
    kvX => y.find(kvX._1 == _._1) match {
      case Some(content) => (kvX._1, kvX._2-content._2)
      case None => kvX
    }
  ).filter(_._2>0)
  // (for {
  //     pair <- x 
  //     pair2 <- y
  //   } if(pair._1 == pair2._1) {yield (pair._1, pair._2-pair2._2)}).sorted
