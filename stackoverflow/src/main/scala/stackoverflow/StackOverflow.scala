...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val qList = postings.filter(_.postingType == 1).map(s => (s.id, s))
    val aList = postings.filter(post => post.postingType == 2 && post.parentId.isDefined).map(s => (s.parentId.get, s))
    qList.join(aList).groupByKey()
  }
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    grouped.map((k,v) => (v.head._1, answerHighScore(v.map(_._2).toArray)))
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    //langSpread
    scored.map( (k,v) => (firstLangInTag(k.tags, langs), v) ).filter( (k,v) => k.isDefined).map( (k,v) => (k.get * langSpread, v)).persist()
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = vectors.map(p => (findClosest(p, means) , p) ).groupByKey().mapValues(averageVectors).collect().toMap
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    //   val medianScore: Int    = ???
      val median = closestGrouped.mapValues { vs =>
      val langId: Int = vs.map(_._1).groupBy(identity).maxBy(_._2.size)._1
      val langLabel: String   = langs(langId / langSpread) // most common language in the cluster
      val clusterSize: Int    = vs.size
      val langPercent: Double = vs.count { case (langIndex, _) => langIndex == langId } * 100d / clusterSize // percent of the questions in the most common language

      val (lower, upper) = vs.map(_._2).toList.sorted.splitAt(clusterSize/ 2)
      val medianScore: Int = if (clusterSize % 2 == 0) (lower.last + upper.head) / 2 else upper.head
