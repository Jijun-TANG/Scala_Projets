...
class Empty extends TweetSet:
  ...
  def union(that: TweetSet): TweetSet = {
    if that.isEmpty then this else that
  }
...
object GoogleVsApple:
  ...
  val allTweetsSet:TweetSet = TweetReader.allTweets

  lazy val googleTweets: TweetSet = allTweetsSet.filter(tweet => google.exists(s => tweet.text.contains(s)))
  lazy val appleTweets: TweetSet = allTweetsSet.filter(tweet => apple.exists(s => tweet.text.contains(s)))
