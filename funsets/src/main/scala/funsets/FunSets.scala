...
trait FunSets extends FunSetsInterface:
  ...
  def emptySet(elem: Int): FunSet = (x:Int) => false
   ...
   */
  def filter(s: FunSet, p: Int => Boolean): FunSet = (x:Int) => if p(x) then s(x) else false
   ...
   */
  def map(s: FunSet, f: Int => Int): FunSet = (x:Int) => exists(s, (k:Int) => x == f(k)) //(i: Int) => exists(s, (j: Int) => i == f(j))
